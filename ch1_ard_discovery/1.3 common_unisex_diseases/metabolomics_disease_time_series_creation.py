""" Obtain per-disease age-of-onset profile and related data for the metabolomics cohort. """
...
def make_age_of_onset_time_series_by_sex(input_df, sex_df, max_age=100):
    """
    Make time series for each sex.
    :param input_df:
    :param sex_df:
    :param max_age:
    :return:
    ...
    for sex in list(input_df_with_sex['sex'].unique()):
        input_df_sex = input_df_with_sex[input_df_with_sex['sex'] == sex]
...
def make_age_of_onset_time_series_by_panukbb_ancestry(input_df, ancestry_df, max_age=100):
    """
    Make time series for each ancestry group as defined by Pan-UKBB ancestry assignments.
    :param input_df:
    :param ancestry_df:
    :param max_age:
    :return:
    """
    disease_time_series_by_ancestry_list = []

    input_df_with_ancestry = merge(input_df, ancestry_df, on='eid')

    for ancestry in list(input_df_with_ancestry['pan_ukbb_ancestry_group'].unique()):
        input_df_ancestry = input_df_with_ancestry[input_df_with_ancestry['pan_ukbb_ancestry_group'] == ancestry]
        disease_time_series_df = make_age_of_onset_time_series(input_df_ancestry, max_age=max_age)
        disease_time_series_df['pan_ukbb_ancestry_group'] = ancestry
        disease_time_series_by_ancestry_list.append(disease_time_series_df)
    return concat(disease_time_series_by_ancestry_list)
...
disease_codes_date_fields_list = [x for x in phenotypes.columns.to_list()[1:] if int(x.split('_')[0].split('p')[1]) % 2 == 0 and 'p13' in x]
...
everyone_info = get_age_of_onset_info_for_common_unisex_diseases(subset_phenotypes=phenotypes,
                                                                 subset_eid_sex=eid_sex,
                                                                 disease_codes_date_fields_list_func=disease_codes_date_fields_list,
                                                                 covariates_func=covariates)
...
for sex in sexes:
    ...
    phenotypes_one_sex = phenotypes[phenotypes['eid'].isin(eids_one_sex)].copy()
...
""" Export common and unisex disease fields in the metabolomics cohort (everyone, by sex, by ancestry group) """
...
common_unisex_diseases.to_csv('~/data/internal/metabolomics/metabolomics_pan_ukbb_eur_common_unisex_disease_date_fields.csv', index=False)
...
for key in per_disease_age_of_onset_info.keys():
    ...
    for disease in specific_disease_df_dict.keys():
        ...
        export_path_dir = '~/data/internal/time_series/metabolomics_pan_ukbb_eur_time_series/common_unisex_' + key.lower()
