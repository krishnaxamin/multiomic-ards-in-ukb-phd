...
olink_assay_version = read_csv('~/data/external/proteomics/olink_assay_version.dat', delim_whitespace=True)  # all version 1
...
npx_eur_genetic = npx_eur[(npx_eur['eid'].isin(pan_ukbb_eur_eids_genomics)) & (~npx_eur['eid'].isin(high_missingness_eids_genomics))]
...
npx_plate_batch_filtered = npx_plate[npx_plate['PlateID'].isin(olink_plates_in_batches_1_to_6)].copy().reset_index(drop=True)
...
sample_missingness = npx_plate_batch_filtered_no_high_missingness_proteins.iloc[:, 1:].isna().sum(axis=1) / (len(npx_plate_batch_filtered_no_high_missingness_proteins.columns) - 2)
high_missingness_samples = list(sample_missingness[sample_missingness > 0.25].index)
npx_plate_batch_filtered_no_high_missingness_proteins_samples = npx_plate_batch_filtered_no_high_missingness_proteins.drop(high_missingness_samples)

""" Merge in more covariate data from Olink data files """
covars = merge(non_npx, npx_plate_batch_filtered_no_high_missingness_proteins_samples[['eid']], on='eid')  # filter to correct EIDs
...
protein_based_covars = protein_based_covars[~protein_based_covars['Assay'].isin([x.upper() for x in high_missingness_proteins])].copy()
...
npx_plate_batch_filtered_no_high_missingness_proteins_samples.drop('PlateID', axis=1).sort_values(by='eid').to_csv('~/data/internal/proteomics/proteomics_pan_ukbb_eur_data.csv', index=False)

""" Export eid """
npx_plate_batch_filtered_no_high_missingness_proteins_samples[['eid']].to_csv('~/data/internal/proteomics/proteomics_pan_ukbb_eur_eids.csv', index=False)

""" Export eid + lifestyle factors """
proteomics_lifestyle = read_csv('ukbiobank/proteomics/proteomics_lifestyles_off_dnanexus.csv')
proteomics_lifestyle_eur = proteomics_lifestyle[proteomics_lifestyle['eid'].isin(list(npx_plate_batch_filtered_no_high_missingness_proteins_samples.eid))].copy()
...
proteomics_lifestyle_eur.loc[proteomics_lifestyle_eur.p23099_i0.isna(), 'p23099_i0'] = np.nanmedian(proteomics_lifestyle_eur['p23099_i0'])
# hours of sleep - continuous
proteomics_lifestyle_eur.loc[~proteomics_lifestyle_eur['p1160_i0'].isin([str(x) for x in range(24)]), 'p1160_i0'] = round(np.nanmedian(proteomics_lifestyle_eur[proteomics_lifestyle_eur['p1160_i0'].isin([str(x) for x in range(24)])]['p1160_i0'].astype(int)))
proteomics_lifestyle_eur.loc[:, 'p1160_i0'] = proteomics_lifestyle_eur.p1160_i0.astype(int)
# Townsend deprivation index - continuous
proteomics_lifestyle_eur.loc[proteomics_lifestyle_eur.p22189.isna(), 'p22189'] = np.nanmedian(proteomics_lifestyle_eur['p22189'])
# education - categorical
proteomics_lifestyle_eur.loc[proteomics_lifestyle_eur['p6138_i0'].isna(), 'p6138_i0'] = 'no_higher_education'
proteomics_lifestyle_eur.loc[proteomics_lifestyle_eur['p6138_i0'].str.contains('College or University degree'), 'p6138_i0'] = 'higher_education'
proteomics_lifestyle_eur.loc[~proteomics_lifestyle_eur['p6138_i0'].str.contains('higher_education'), 'p6138_i0'] = 'no_higher_education'
# physical activity - continuous
proteomics_lifestyle_eur.loc[~proteomics_lifestyle_eur['p884_i0'].isin([str(x) for x in range(8)]), 'p884_i0'] = round(np.nanmedian(proteomics_lifestyle_eur[proteomics_lifestyle_eur['p884_i0'].isin([str(x) for x in range(8)])]['p884_i0'].astype(int)))
proteomics_lifestyle_eur.loc[:, 'p884_i0'] = proteomics_lifestyle_eur.p884_i0.astype(int)
# grip strength - continuous
proteomics_lifestyle_eur.loc[proteomics_lifestyle_eur.p46_i0.isna(), 'p46_i0'] = np.nanmedian(proteomics_lifestyle_eur.p46_i0)
proteomics_lifestyle_eur.loc[proteomics_lifestyle_eur.p47_i0.isna(), 'p47_i0'] = np.nanmedian(proteomics_lifestyle_eur.p47_i0)
...
for field in ['p' + str(1329 + 10*x) + '_i0' for x in range(7)]:
    proteomics_lifestyle_eur.loc[proteomics_lifestyle_eur[field].isna(), field] = 'Do not know'
    proteomics_lifestyle_eur.loc[proteomics_lifestyle_eur[field].isin(['Prefer not to answer', 'Do not know']), field] = 'Unanswered'
# export
proteomics_lifestyle_eur.drop(['p48_i0', 'p49_i0'], axis=1).to_csv('~/data/internal/proteomics/proteomics_pan_ukbb_eur_lifestyles.csv', index=False)

""" Filter phenotype data to new eid set """
proteomics_phenotypes = read_csv('~/data/external/proteomics/proteomics_phenotype_file_off_dnanexus.csv', low_memory=True)
proteomics_phenotypes = proteomics_phenotypes[proteomics_phenotypes['eid'].isin(list(covars['eid']))]
proteomics_phenotypes.to_csv('~/data/internal/proteomics/proteomics/proteomics_pan_ukbb_eur_phenotypes.csv', index=False)
