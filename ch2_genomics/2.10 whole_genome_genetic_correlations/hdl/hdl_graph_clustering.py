"""
Perform graph-based Louvain clustering FDR-corrected significant HDL values.
...
hdl = read_csv('~/data/internal/genomics/whole_genome_genetic_correlations/hdl_genetic_correlations.csv')
...
# remove 'infinitely' correlated disease pairs
hdl = hdl[~hdl['genetic_correlation'].isin([np.inf, -np.inf])].copy()

# FDR correction and filtering
fdr_correction = smm.multipletests(hdl['pval'], alpha=0.05, method='fdr_bh')
hdl_sig = hdl[fdr_correction[0]]

""" Set up graph """

hdl_for_graph = merge(hdl_sig,
                      disease_info[['disease_field', 'code_chapter']].rename(columns={'disease_field': 'disease1',
                                                                                      'code_chapter': 'disease1_code_chapter'}),
                      on='disease1')
hdl_for_graph = merge(hdl_for_graph,
...
all_edges = []
for i in range(len(hdl_for_graph)):
    disease1 = hdl_for_graph.at[i, 'disease1_code_chapter']
    disease2 = hdl_for_graph.at[i, 'disease2_code_chapter']
    cor = hdl_for_graph.at[i, 'genetic_correlation']
...
disease_groups = [[node_disease_mapper[x] for x in group] for group in [list(x) for x in subclusters3]]
...
for group in disease_groups:
    ...
    for combo in group_diseases_combinations:
        corr_row = hdl_for_graph.loc[(hdl_for_graph['disease1_code_chapter'] == sorted(combo)[0]) & (
                    hdl_for_graph['disease2_code_chapter'] == sorted(combo)[1]), 'genetic_correlation']
        if len(corr_row) > 0:
...
cluster_combinations_idxs_hdl = []
for combo in cluster_combinations:
    corr_row = hdl_for_graph.loc[(hdl_for_graph['disease1_code_chapter'] == sorted(combo)[0]) & (
                hdl_for_graph['disease2_code_chapter'] == sorted(combo)[1])]
    if len(corr_row) > 0:
        cluster_combinations_idxs_hdl.append(corr_row.index.values[0])

non_cluster_corrs = hdl_for_graph.drop(cluster_combinations_idxs_hdl)['genetic_correlation'].values
...
for group in disease_groups:
    ...
    group_alpha_hats = []
    for combo in group_diseases_combinations:
        combo_multimorbidity = multimorbidity.loc[
            (multimorbidity['disease1'] == combo[0]) & (multimorbidity['disease2'] == combo[1]), 'alpha_hat']
        if len(combo_multimorbidity) == 0:
            continue
        alpha_hat = combo_multimorbidity.values[0]
...
for combo in cluster_combinations:
    combo_multimorbidity = multimorbidity.loc[
        (multimorbidity['disease1'] == combo[0]) & (multimorbidity['disease2'] == combo[1])]
    if len(combo_multimorbidity) == 0:
        continue
    cluster_combinations_idxs_alphamle.append(combo_multimorbidity.index.values[0])
...
disease_group_df[['disease_group', 'sig_greater_correlation', 'sig_greater_multimorbidity']].to_csv(
    '~/data/internal/genomics/whole_genome_genetic_correlations/hdl_disease_groups.csv', index=False)
