...
if (length(protein_result_files) == 2920){
  ...
  for (file in list.files(protein_result_dir, full.names = TRUE)){
    all_firth_results <- rbind(all_firth_results, read.csv(file))
  }
  write_csv(all_firth_results, paste0('~/ch3_proteomics/3.7 adjusting_for_prior_diagnoses/association_analyses/firth_results/', disease, '_all_proteins.csv'))
