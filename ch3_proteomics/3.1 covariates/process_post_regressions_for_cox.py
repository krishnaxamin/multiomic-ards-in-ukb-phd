...
for protein in protein_fields:
    residual = read_csv('~/ch3_proteomics/3.1 covariates/regressing_out_for_cox/post_regression_protein_data/' + protein + '.csv')
...
scaled_residuals.to_csv('~/data/internal/proteomics/proteomics_pan_ukbb_eur_post_regressions_for_cox_robust_scaled_data.csv', index=False)
...
for protein in protein_fields:
    var_explained = read_csv('~/ch3_proteomics/3.1 covariates/regressing_out_for_cox/effects/' + protein + '_effects_during_regressions.csv')
    vars_explained_list.append(var_explained)
    bar.update()
vars_explained_by_all_covars_at_each_step = concat(vars_explained_list)
vars_explained_by_all_covars_at_each_step.to_csv('~/ch3_proteomics/3.1 covariates/regressing_out_for_cox/effects_during_regressions_for_cox.csv', index=False)

# plot lifestyle effects over 'time'
vars_explained_by_all_covars_at_each_step = read_csv('~/ch3_proteomics/3.1 covariates/regressing_out_for_cox/effects_during_regressions_for_cox.csv')
...
sns.lineplot(vars_explained_by_all_covars_at_each_step, x='stage', y='var_explained', hue='covar_name', estimator='mean', errorbar='sd',
             hue_order=['Well', 'Date processed', 'Plate row', 'Plate column', 'Oily fish intake', 'Non-oily fish intake', 'Processed meat intake', 'Beef intake', 'Lamb intake', 'Pork intake', 'Water intake', 'Storage time'])
...
fig.tight_layout()
fig.savefig('~/ch3_proteomics/3.1 covariates/plots/effects_during_regressions_for_cox.png')
fig.savefig('~/ch3_proteomics/3.1 covariates/plots/effects_during_regressions_for_cox.svg')
