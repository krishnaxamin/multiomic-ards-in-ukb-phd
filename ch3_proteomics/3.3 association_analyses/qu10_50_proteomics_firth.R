# Lifestyle-adjusted (with collinear variables removed) Firth regression for proteomics
library(logistf)
library(tidyverse)
library(lubridate)
library(progress)
...
data <- read_csv('~/data/internal/proteomics/proteomics_pan_ukbb_eur_post_regressions_robust_scaled_data.csv', num_threads = 1, show_col_types = FALSE)
...
for (protein in proteins){
  ...
  # export Firth model
  # saveRDS(firth_model, paste0('~/proteomics/association_analysis/firth_lifestyle/robjs/', protein, '_', disease, '.rds'))
...
write_csv(all_firth_results, paste0('~/ch3_proteomics/3.3 association_analyses/firth/', disease, '_all_proteins.csv'))
