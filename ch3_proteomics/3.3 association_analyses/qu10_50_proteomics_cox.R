# Lifestyle-adjusted (with collinear variables removed) Cox regression for proteomics
library(tidyverse)
library(Coxmos)
library(lubridate)
library(progress)
...
# if analysis already complete, stop script
if (file.exists(paste0('~/ch3_proteomics/3.3 association_analyses/cox/', disease, '_all_proteins.csv'))){
  print(paste0(Sys.time(), ': Analysis already complete.'))
  flush.console()
  Sys.sleep(0.1)
  stop()
}
...
data <- read_csv('~/data/internal/proteomics/proteomics_pan_ukbb_eur_post_regressions_robust_scaled_data_for_cox.csv', num_threads = 1, show_col_types = FALSE)

# get vector of proteins
...
# isolate columns to be used from data_matrix
data_matrix <- data_matrix %>%
  select(all_of(columns_for_cox))
...
for (protein in proteins){
  # remove samples with NA protein values, and select phenotype for this one disease
  ...
  cox_results <- rbind(cox_results, model_result_df)
  
  bar$tick()
}

write_csv(cox_results, paste0('~/ch3_proteomics/3.3 association_analyses/cox/', disease, '_all_proteins.csv'))

print(paste0(Sys.time(), ': Analyses complete. Results exported.'))
flush.console()
Sys.sleep(0.1)
