' Getting lifestyle effects on metabolomics data, using linear regression '

library(tidyverse)

task_number <- as.integer(commandArgs(trailingOnly = TRUE)[1])

covars <- read.csv('~/data/internal/metabolomics/metabolomics_pan_ukbb_eur_lifestyles.csv')

metabolites <- read.csv('~/data/internal/metabolomics/metabolite_fields.csv')$metabolite_field
metabolite <- metabolites[task_number]
metabolite_data <- read.csv('~/data/internal/metabolomics/metabolomics_pan_ukbb_eur_log1p_data.csv')[c('eid', metabolite)]
...
metabolite_vars_explained <- data.frame()
...
for (x in lifestyle_info){
  ...
  model_result <- var_explained_by_covar(covars_df = covars,
                                         response_var_df = metabolite_data,
                                         covar = x$field,
  ...
  metabolite_vars_explained <- rbind(metabolite_vars_explained, model_result_df)
}

write_csv(metabolite_vars_explained, paste0('lifestyle_effects/on_metabolites/', metabolite, '_lifestyle_effects_on_metabolites.csv'))
